...
import { SocketEvents, GameAction, DebugCommand, CreateRoomOptions, UpdateSettingsPayload } from '@sq/shared';
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on(SocketEvents.CREATE_ROOM, ({ playerName, userId, options }: { playerName: string, userId: string, options?: CreateRoomOptions }, callback: (roomId: string) => void) => {
    try {
      const roomId = roomManager.createRoom((state) => {
        io.to(state.roomId).emit(SocketEvents.GAME_STATE_UPDATE, state);
      }, options);
      // Fix: Ensure player is added with CORRECT socket ID
  ...
  socket.on(SocketEvents.JOIN_ROOM, ({ roomId, playerName, userId }: { roomId: string, playerName: string, userId: string }) => {
    try {
      // Fix: Ensure player is updated with NEW socket ID on reconnect
      const game = roomManager.joinRoom(roomId, userId, playerName, socket.id);
      socket.join(roomId);
      io.to(roomId).emit(SocketEvents.GAME_STATE_UPDATE, game.getState());
      broadcastRoomList();
  ...
  socket.on(SocketEvents.PLAY_ACTION, ({ roomId, action }: { roomId: string, action: GameAction }) => {
    ...
    try {
      // Verify player ownership of socket
      // We need to find the player that corresponds to this action.
      // But wait, action.playerId should match the player associated with this socket.
      // We can look up player by socket.id
      ...
      // Force action to use the correct player ID (security)
  ...
  socket.on('start_game', (roomId: string) => {
      ...
      if (game) {
          try {
             // Check if sender is host? For MVP anyone can start.
             game.startGame();
  ...
  socket.on(SocketEvents.UPDATE_GAME_SETTINGS, (payload: any) => {
      // Use explicit parsing to avoid any destructuring issues
  ...
  socket.on(SocketEvents.DEBUG_COMMAND, ({ roomId, command }: { roomId: string, command: DebugCommand }) => {
